...
.get('/guildmaster/quete', function(req, res) { 
     connection.query("SELECT idQuest, experience, name, summary, duree, reward FROM quest", function(err, rows, fields){
	if (!err){
	   //console.log(rows);
...
.get('/guildmaster/quete/commencer/:id/:name', function(req, res) {
     connection.query("SELECT idQuest, experience, name, summary, duree, reward FROM quest where idQuest ="+ req.params['id'] +" and name = '"+ req.params['name'] +"'", function(err, rows, fields){
	if (!err){
	   var quest = rows;
